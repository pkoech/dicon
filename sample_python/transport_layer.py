...
class StudentUSocket(StudentUSocketBase):
  ...
  def connect(self, ip, port):
    ...
    syn_packet.tcp.seq = self.snd.iss
    self.tx(syn_packet)
    self.state = SYN_SENT
  ...
  def tx(self, p, retxed=False):
    ...
    # while not self.retx_queue.empty():
    #     p = self.retx_queue.peek()
    #     p.retxed = True
    if not p.retxed and len(p.tcp.payload) > 0:
        p.tx_ts = self.stack.now
  ...
  def maybe_send(self):
    ...
    while remaining > 0:
        in_flight = snd.nxt|MINUS|snd.una
        remaining_window= self.snd.wnd|MINUS|in_flight
        if remaining_window <= 0:
            break
        payload = min(remaining,self.mss,remaining_window)
        p = self.new_packet(ack=True, data=self.tx_data[:payload])
        self.tx(p)
        self.tx_data = self.tx_data[payload:]
        remaining = remaining - payload
        num_pkts += 1
        bytes_sent = bytes_sent |PLUS| payload
